...
WEAK_TEXT_THRESHOLD = 40
WEAK_LINK_CONFIDENCE_THRESHOLD = 0.5
...
def _extract_text_fields(row: dict[str, Any]) -> tuple[str, str]:
    summary = _normalize_whitespace(
        row.get("summary_text") or row.get("raw_text_or_summary") or row.get("summary")
    )
    excerpt = _normalize_whitespace(
        row.get("excerpt_text") or row.get("excerpt") or row.get("snippet")
    )
...
def _normalize_outbound_links(raw_links: Any) -> tuple[list[dict[str, Any]], int]:
    ...
    for link in raw_links:
        ...
        url = _normalize_url(candidate.get("url") or candidate.get("href"))
        ...
        normalized_links.append(
            {
                ...
                "label": _normalize_whitespace(
                    candidate.get("label") or candidate.get("title") or candidate.get("text")
                ),
                "type": _normalize_whitespace(candidate.get("type") or candidate.get("rel"))
                or "reference",
                "extraction_confidence": _to_float(
                    candidate.get("extraction_confidence", 0.5),
                    0.5,
                ),
            }
        )
    return normalized_links, malformed_count
...
def _normalize_row(settings: Settings, row: dict[str, Any]) -> dict[str, Any]:
    ...
    summary_text, excerpt_text = _extract_text_fields(row)
    outbound_links, malformed_outbound_links = _normalize_outbound_links(
        row.get("outbound_links")
    )
    normalized["summary_text"] = summary_text
    normalized["excerpt_text"] = excerpt_text
    normalized["outbound_links"] = outbound_links
    normalized["malformed_outbound_links"] = malformed_outbound_links
    return normalized
...
def _as_timestamp_or_none(raw_value: str | None) -> datetime | None:
    if not raw_value:
        return None
    try:
        return _parse_upstream_ts(raw_value)
    except (TypeError, ValueError):
        return None
...
def _apply_evidence_quality(
    ...
    if quality_state in ("quarantined", "stale_source", "dead_letter"):
        return validator

    outbound_links = row.get("outbound_links") or []
    malformed_outbound_links = int(row.get("malformed_outbound_links") or 0)
    summary_text = _normalize_whitespace(row.get("summary_text"))
    excerpt_text = _normalize_whitespace(row.get("excerpt_text"))
    has_evidence_text = bool(summary_text or excerpt_text)
    has_links = len(outbound_links) > 0
    weak_text = bool(summary_text and len(summary_text) < WEAK_TEXT_THRESHOLD) or bool(
        excerpt_text and len(excerpt_text) < WEAK_TEXT_THRESHOLD
    )
    weak_links = has_links and all(
        _to_float(link.get("extraction_confidence"), 0.5) < WEAK_LINK_CONFIDENCE_THRESHOLD
        for link in outbound_links
    )

    if malformed_outbound_links > 0:
        quality_state = "quarantined"
        reason_code = "malformed_outbound_links"
        last_error_code = "malformed_outbound_links"
    elif not has_evidence_text and not has_links:
        quality_state = "metadata_incomplete"
        reason_code = "missing_evidence_fields"
    elif weak_text or (weak_links and not has_evidence_text):
    ...
    with connect(settings.db_url) as conn:
        with conn.cursor() as cur:
            for row in fixture_rows:
                ...
                validator = _apply_evidence_quality(
                    normalized_row,
                    _validate_envelope(normalized_row, settings, now),
                )
                ...
                params = {
                    ...
                    "published_at": _as_timestamp_or_none(normalized_row.get("published_at")),
                    ...
                    "summary_text": normalized_row.get("summary_text") or None,
                    "excerpt_text": normalized_row.get("excerpt_text") or None,
                    "outbound_links_json": json.dumps(
                        normalized_row.get("outbound_links", []),
                        ensure_ascii=False,
                    ),
